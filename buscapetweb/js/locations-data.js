...
var BuscapetLocations = window.BuscapetLocations = {
  ...
  getCountryByCodeOrName(val) {
    if (!val) return null;
    const v = val.toLowerCase().trim();
    return this.countries.find(c => c.code.toLowerCase() === v || c.name.toLowerCase() === v || c.name.toLowerCase().includes(v));
  },

  getStates(val) {
    const country = this.getCountryByCodeOrName(val);
    return country ? country.states.sort((a, b) => a.name.localeCompare(b.name)) : [];
  },

  getCities(countryVal, stateName) {
    const country = this.getCountryByCodeOrName(countryVal);
    if (!country) return [];
